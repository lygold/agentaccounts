...
export async function submitOwnerLawyer(formData: FormData) {
  try {
    ...
    await advanceDraft(session.agentId, "owner-lawyer", {
      ownerLawyer: {
    ...
    console.error("submitOwnerLawyer failed:", e);
    redirect(stepHref("owner-lawyer") + "?error=save");
  }
}
