...
export async function submitBuyerLawyer(formData: FormData) {
  try {
    ...
    await advanceDraft(session.agentId, "buyer-lawyer", {
      buyerLawyer: {
    ...
    console.error("submitBuyerLawyer failed:", e);
    redirect(stepHref("buyer-lawyer") + "?error=save");
  }
}
