...
export async function submitBuyerAgent(formData: FormData) {
  try {
    ...
    await advanceDraft(session.agentId, "buyer-agent", {
      buyerAgent: {
    ...
    console.error("submitBuyerAgent failed:", e);
    redirect(stepHref("buyer-agent") + "?error=save");
  }
}
